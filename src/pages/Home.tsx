    ...
    <div className="pt-20" ref={containerRef}>
      ...
      <section className="min-h-[90vh] flex flex-col justify-center px-6 relative overflow-hidden">
        <motion.div 
          ...
          className="absolute top-1/4 right-10 w-96 h-96 bg-accent/10 blur-[120px] rounded-full pointer-events-none" 
        />
        ...
        <div className="max-w-7xl mx-auto w-full grid lg:grid-cols-2 gap-12 items-center">
          <div className="z-10">
            ...
            <h1 className="text-[clamp(48px,8vw,96px)] leading-[1.05] mb-8 flex flex-col">
              ...
              <div className="overflow-hidden py-1">
                <motion.span
                  ...
                  className="block text-gradient origin-bottom-left"
                >
                  DIGITAL
                </motion.span>
              </div>
          ...
          >
            <motion.div 
              ...
              className="glass p-6 rounded-lg relative z-10 shadow-2xl border border-border-color backdrop-blur-xl"
            >
              ...
              <div className="font-mono text-sm text-accent/80 leading-relaxed">
                <p><span className="text-text-primary">const</span> clinic = <span className="text-text-primary">{'{'}</span></p>
                <p className="pl-4">name: <span className="text-text-primary">"SmileCare Dental"</span>,</p>
                <p className="pl-4">conversion: <span className="text-text-primary">"45%"</span>,</p>
                <p className="pl-4 text-text-primary">status: <span className="text-accent">"Thriving"</span></p>
                <p><span className="text-text-primary">{'}'}</span>;</p>
                <p className="mt-4 text-text-primary">clinic.optimizeForPatients();</p>
              </div>
            </motion.div>
      ...
      {/* Trust Marquee */}
      <div className="py-12 border-y border-border-color bg-surface/30 overflow-hidden">
        <div className="flex animate-marquee whitespace-nowrap">
          {[...Array(10)].map((_, i) => (
            <div key={i} className="flex items-center gap-12 mx-12">
              <span className="text-2xl font-display font-bold text-text-primary/20 uppercase tracking-tighter">Modern Dental Web Design</span>
              <span className="text-2xl font-display font-bold text-accent/40 uppercase tracking-tighter">High Conversion Design</span>
              <span className="text-2xl font-display font-bold text-text-primary/20 uppercase tracking-tighter">Dental SEO Experts</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <section className="py-32 px-6 max-w-7xl mx-auto">
        <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
          ...
          <Link to="/work" className="group flex items-center gap-2 text-accent font-bold uppercase tracking-widest hover:text-text-primary transition-colors">
        ...
        <div className="grid md:grid-cols-10 gap-8">
          {featuredWork.map((project, idx) => (
            ...
            >
              ...
              <motion.div 
                className="absolute bottom-0 left-0 p-8 w-full translate-y-4 group-hover:translate-y-0 transition-transform duration-500"
              >
                <p className="text-accent text-xs font-bold uppercase tracking-widest mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">{project.category}</p>
                <h3 className="text-3xl font-display font-bold text-text-primary mb-4">{project.title}</h3>
                <a href={project.link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 text-text-secondary hover:text-text-primary transition-colors text-sm font-bold uppercase tracking-widest">
                  View Live Site <ArrowRight size={16} className="group-hover:translate-x-2 transition-transform" />
                </a>
              </motion.div>
            </motion.div>
          ))}
      ...
      {/* Why Choose Us */}
      <section className="py-32 bg-surface/20 border-y border-border-color">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {benefits.map((benefit, idx) => (
              <motion.div
                ...
                className="glass p-8 rounded-lg text-center hover:border-accent/50 transition-all duration-300 shadow-lg hover:shadow-accent/20"
              >
                <div className="mx-auto w-12 h-12 bg-accent/10 rounded-full flex items-center justify-center mb-6 text-accent group-hover:scale-110 transition-transform">
                  <benefit.icon size={24} />
                </div>
                <h3 className="text-xl font-bold mb-3 text-text-primary">{benefit.title}</h3>
      ...
      <section className="py-32 px-6 max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-4 gap-px bg-white/5 border-thin">
          {processSteps.map((step, idx) => (
            ...
            >
              <div className="card-uiverse-content p-10 group overflow-hidden relative">
                <div className="absolute inset-0 bg-gradient-to-br from-accent/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
                <span className="text-accent font-mono text-xl mb-8 block transform group-hover:-translate-y-2 transition-transform duration-300">0{idx + 1}</span>
                <h3 className="text-2xl mb-4 group-hover:text-accent transform group-hover:translate-x-2 transition-all duration-300">{step.title}</h3>
